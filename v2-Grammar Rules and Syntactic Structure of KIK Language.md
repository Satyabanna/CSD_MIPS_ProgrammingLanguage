---
title: v2-Grammar Rules and Syntactic Structure of KIK Language

---

---
title: Grammar Rules and Syntactic Structure of KIK Language
...
```ebnf
<program> ::= { <import> }* { <declaration> | <function> }* <main-function> ;
...
Custom `io.kik` functions: `input()` (returns string), `output(message)`.

```ebnf
<io-statement> ::= "output" "(" <expression> { "," <expression> }* ")" ";"
| <identifier> "=" "input" "(" ")" ";"
```
...
Example: `if score >= 90 { output("Grade: A"); } else if score >= 80 { ... } else { ... }`
...
```ebnf
<while-loop> ::= "while" <expression> "{" <statement>* "}"},{all:true,find:
```

Example: `while count <= 5 { output(count); count++; }`
...
Example: `do { output(count); count++; } while (count <= 5); (empty block after colon).
...
Example: `for (int count = 1; count <= 5; count++) { output(count); }`
...
int kik() {
    ## Function call
    int sum = add(5, 7);
    output("The sum is: ", sum);  ## Output: 12
    return 0;
}
```
...
int kik() {
    ...
    output("The area of the rectangle is: ", area);  ## Output: 50
    return 0;
}
```
### Return Keyword
...
int kik() {
    int result = square(4);
    output("The square of 4 is: ", result);  ## Output: 16
    return 0;
}
```
### Pass By Reference
...
int kik() {
    int number = 10;
    output("Original number: ", number);  ## Output: 10
    
    increment(number);  ## Pass by reference
    
    output("Modified number after increment: ", number);  ## Output: 11
    return 0;
}
```
### Variable Scope
...
int kik() {
    ...
    output("Local variable: ", localVar);  ## Output: 10
    return 0;
}
```
...
int kik() {
    output("Global variable: ", globalVar);  ## Output: 20
    return 0;
}
```
...
void display(int param) {
    output("Parameter value: ", param);  ## Output: 30
}
...
int kik() {
    ...
    output("Sum: ", result);  ## Output: 8
    output("Double sum: ", doubleSum);  ## Output: 16
    output("Function called ", globalCounter, " time(s)");  ## Output: 1
...
int kik() {
    ...
    output( "The numeric value of 'Wednesday' is: " ,  Wednesday ); // Outputs: 3
...
int kik() {
    ...
    output( "Integer: " ,  intValue ,  ", Float: " ,  floatValue );  ## Outputs: Integer: 5, Float: 5.0
    return 0;               ## Indicate successful execution
}
```
...
int kik() {
    ...
    output( "Float: " ,  floatValue ,  ", Integer: " ,  intValue );  ## Outputs: Float: 7.8, Integer: 7
    return 0;                ## Indicate successful execution
}
```
...
int kik() {
    ...
    try:
        ...
        output( "Result: " ,  result );  ## Outputs result if no exception
    :
    catch (str error):
        output( "Error: " ,  error );  ## Outputs: Error: Zero divisor
    :
    return 0;  ## Indicate successful execution
}
```
...
int kik() {
    // Data to write to the filegggggggg
...
int b = 90;
int result = a + b;
output( "Final output after adding: " ,  result );
return 0;
}
```
...
int result = (a * 2) + b;
if result == 25 {
output( "Expression evaluated correctly!" );
} else {
output( "Expression evaluation failed." );
}
return 0;
}
```

### Test Case 5: Standard Input/Output (input, output)
This test demonstrates how to prompt a user for input and read it using input, then display a formatted message using output.put.
```
...
if num < 0 {
output( "Error: Please enter a non-negative number!" );
return 1;
}
...
This section details the Object-Oriented Programming (OOP) features for the KIK language, following the C-style syntax updates. The design is based on classes, inheritance, and encapsulation, with function and constructor declarations now being colon-less.

## 15.1. Feature Specification

### 15.1.1. Classes and Objects

A class is a blueprint for creating objects. The class definition begins with the `class` keyword, followed by the class name, and a body enclosed in curly braces (`{}`). A semicolon is required after the closing brace.
...
### 15.1.2. Constructors and Destructors

Constructors and destructors are special methods for object initialization and cleanup. They follow the new colon-less function declaration syntax.
...
### 15.1.3. Object Creation and Member Access

Objects are created using the `new` keyword. Members are accessed using the dot (`.`) operator.
...
### 15.1.4. Access Modifiers (public, private)

Members are grouped into `public` and `private` sections, which are introduced by the keyword followed by a colon.

**Example: Class with Private Data and Public Accessors**
```kik
import "io.kik";
...
Inheritance allows a derived class to inherit properties and methods from a base class. The syntax is `class Derived : Base { ... }`.
...
### 15.1.6. Constructor and Inheritance Combined

**Example: Constructor + Inheritance Combined**
```kik
import "io.kik";
...
class Dog : Animal {
    public:
        // Note: In KIK, base class constructor calls are not explicitly shown
        // in the derived class constructor, so we assume the derived class
        // is responsible for setting inherited members.
        Dog(string n) {
            name = n;
        }
...
## 15.2. Grammar Rule Additions (EBNF)

The following rules should be added to the main KIK grammar to support OOP features, reflecting the new syntax.
...
<inheritance_specifier> ::= ":" <identifier> ;
...
## 15.3. Full OOP Program Example

This program demonstrates a complete use case of classes, inheritance, and methods.
...
class Manager : Employee {
    ...
    public:
        Manager(string n, int i, float s, int t) {
            // Assuming inherited members are set directly in the derived constructor
            name = n;
            id = i;
            salary = s;
            teamSize = t;
...
int main() {
    Manager m = new Manager("Akash", 101, 55000.5, 5);
    m.showDetails();
    return 0;
}
...
int kik() {
    ...
    if result > 10 {
        output( "Result is greater than 10: " ,  result );
    } else {
        output( "Result is 10 or less: " ,  result );
    }
...
int kik() {
    ...
    output( "Factorial of 5: " ,  fact );
    
    // Pass by reference
    int value = 10;
    output( "Original value: " ,  value );
    increment(value);
    output( "After increment: " ,  value );
    
    output( "Function called " ,  globalCounter ,  " times" );
...
int kik() {
    ...
    try:
        ...
        output( "Division result: " ,  result );
    :
    catch (str error):
        output( "Math error: " ,  error );
    :
...
int kik() {
    ...
    output( "Implicit cast: " ,  intValue ,  " -> " ,  floatValue );
    
    // Explicit type casting
    floatValue = 7.8;
    intValue = (int)floatValue;
    output( "Explicit cast: " ,  floatValue ,  " -> " ,  intValue );
    
    // Dynamic memory allocation
    int$ ptr: new int;
    *ptr = 42;
    output( "Dynamic value: " ,  *ptr );
    delete ptr;
...
### Test Case 5: Standard Input/Output (input, output)
This test demonstrates how to prompt a user for input and read it using input, then display a formatted message using output.
```
Expected Output (Interactive):
The program will prompt for a name and age, then print a personalized greeting.
```
